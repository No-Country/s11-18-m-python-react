...
const ContentTextRutine = [
  ...
  },
  {
    ...
    // Puedes agregar más propiedades específicas para el tipo de video
  },
];
...
const VideoFaqRutineCont = ({ title, description, icon, iconText }) => {
  ...
  return (
    ...
    <div className='w-full flex justify-center items-center'>
        <span className=' py-2 px-2 w-1/2 flex justify-evenly items-center text-Turquesa/600 text-lg'> <BsPlus className='border-2 rounded-full text-2xl border-Turquesa/600'/> Más videos </span>
    </div>
      </div>
      ...
      {ContentTextRutine.map((routine) => (
        <article key={routine.id} className="border-t-[1px] border-Grises/200 text-white my-5">
          {routine.type === "content-video" ? (
            ...
            <FaqRutineCont key={routine.id} more={routine.more} content={routine.description} title={routine.title} />
          )}
        </article>
      ))}
    </section>
