...
class CommmentPostSerializer(serializers.ModelSerializer):
    ...
    def hide_comment(self, validated_data):
        ...
        existing_post = Posts.objects.filter(id=post_id).first()
        existing_comment = CommentPost.objects.filter(
            comment_post=post_id, comment_user=user, id=comment_id
        ).first()
...
class LikeSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        existing_like = Junction_likes.objects.filter(
            likes_post=post_id, likes_user=user
        ).first()
        existing_post = Posts.objects.filter(id=post_id).first()
...
class RepostSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        existing_repost = Junction_repost.objects.filter(
            repost_post=post_id, repost_user=user
        ).first()
        existing_post = Posts.objects.filter(id=post_id).first()
